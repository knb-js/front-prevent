...
public class ComunaClient {
	...
	public List<ComunaDTO> findAllComuna() {	
		try {
			...
			ResponseEntity<ResponseDTO> result = rest.getForEntity(configClientValues.getUrlBackend() + "/comuna", ResponseDTO.class);
			return Arrays.asList(om.readValue(om.writeValueAsBytes(result.getBody().getData()), ComunaDTO[].class));
		} catch (Exception e) {
			return null;
		}
	}
