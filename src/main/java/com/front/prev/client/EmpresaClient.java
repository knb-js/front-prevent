...
public class EmpresaClient {
	...
	public List<EmpresaDTO> findAllEmpresa() {	
		try {
			...
			ResponseEntity<ResponseDTO> result = rest.getForEntity(configClientValues.getUrlBackend() + "/empresa", ResponseDTO.class);
			return Arrays.asList(om.readValue(om.writeValueAsBytes(result.getBody().getData()), EmpresaDTO[].class));
		} catch (Exception e) {
			return null;
		}
	}
