...
@Component
@EnableAsync
public class EstadoContratoClient {
	...
	public List<EstadoContratoDTO> findAllEstadoContrato() {	
		try {
			...
			ResponseEntity<ResponseDTO> result = rest.getForEntity(configClientValues.getUrlBackend() + "/estadoContrato", ResponseDTO.class);
			return Arrays.asList(om.readValue(om.writeValueAsBytes(result.getBody().getData()), EstadoContratoDTO[].class));
		} catch (Exception e) {
			return null;
		}
	}
