...
public class TipoPlanClient {
	...
	public List<TipoPlanDTO> findAllTipoPlan() {	
		try {
			...
			ResponseEntity<ResponseDTO> result = rest.getForEntity(configClientValues.getUrlBackend() + "/Planes", ResponseDTO.class);
			return Arrays.asList(om.readValue(om.writeValueAsBytes(result.getBody().getData()), TipoPlanDTO[].class));
		} catch (Exception e) {
			return null;
		}
	}
