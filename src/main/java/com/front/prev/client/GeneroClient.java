...
private final RestTemplate rest = new RestTemplate();
	...
	public List<GeneroDTO> findAllGenero() {	
		try {
			...
			ResponseEntity<ResponseDTO> result = rest.getForEntity(configClientValues.getUrlBackend() + "/genero", ResponseDTO.class);
			return Arrays.asList(om.readValue(om.writeValueAsBytes(result.getBody().getData()), GeneroDTO[].class));
		} catch (Exception e) {
			return null;
		}
	}
