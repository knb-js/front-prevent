...
@Component
@EnableAsync
public class RepresentanteClient {
	...
	public List<RepresentanteDTO> findAllRepresentante() {	
		try {
			...
			ResponseEntity<ResponseDTO> result = rest.getForEntity(configClientValues.getUrlBackend() + "/representante", ResponseDTO.class);
			return Arrays.asList(om.readValue(om.writeValueAsBytes(result.getBody().getData()), RepresentanteDTO[].class));
		} catch (Exception e) {
			return null;
		}
	}
