...
public class RubroClient {
	...
	public List<RubroDTO> findAllRubro() {	
		try {
			...
			ResponseEntity<ResponseDTO> result = rest.getForEntity(configClientValues.getUrlBackend() + "/rubro", ResponseDTO.class);
			return Arrays.asList(om.readValue(om.writeValueAsBytes(result.getBody().getData()), RubroDTO[].class));
		} catch (Exception e) {
			return null;
		}
	}
